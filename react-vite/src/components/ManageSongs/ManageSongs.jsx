...
const ManageSongs = () => {
  ...
  const handleSaveAlbum = (albumId) => {
    ...
    if (
      albumInputValues[albumId]?.name &&
      albumInputValues[albumId].name !== originalAlbum.name
    ) {
    ...
    if (
      ...
      updatedData.releaseDate = new Date(originalAlbum.releaseDate)
        .toISOString()
        .split("T")[0];
    }
  ...
  const handleSaveSongs = (albumId) => {
    ...
    albumSongs.forEach((song) => {
      ...
      if (
        songInputValues[song.id]?.name &&
        songInputValues[song.id].name !== song.name
      ) {
  ...
  return (
    <div className="flex flex-col mx-auto mt-6 w-4/5 py-6 items-center min-h-[70vh]">
      ...
      {!albums.length && <h2 className="text-2xl mb-3">No Songs Yet</h2>}
      <div className="grid grid-cols-2 gap-4">
        {albums.map((album) => (
          <div key={album.id} className="bg-card rounded-md">
            <div className="flex">
                ...
                {editingAlbum === album.id ? (
                  <div className="flex flex-col gap-2">
                    <input
                      ...
                      onChange={(e) =>
                        handleInputChange(album.id, "name", e.target.value)
                      }
                    ...
                    <input
                      type="date"
                      value={
                        new Date(albumInputValues[album.id].releaseDate)
                          .toISOString()
                          .split("T")[0]
                      }
                      onChange={(e) =>
                    ...
                    .map((song) => (
                      <div key={song.id} className="flex items-center mt-2">
                        ...
                        {editingSongs[album.id] && (
                          <button
                            onClick={() =>
                              handleOpenDeleteModal(song.id, "song")
                            }

...
export const createArtist = (artistData) => async (dispatch) => {
  const artist = await post("/artists", artistData); //This will throw an error if there is an error
  dispatch(loadOne(artist));
  return artist;
};

export const selectArtists = (state) => state.artists;
export const selectArtistById = (artistId) => (state) =>
